...
**Run Name:** 3_2026-06-30_08-29-28
**Run Number:** 3
**Date:** June 30, 2026 at 08:29 AM
...
```py
"""
generate_outputs.py  —  HCRecov Report Orchestrator
=====================================================
Single entry point to produce both PDF and PPT outputs.

Usage:
    python generate_outputs.py           # both PDF + PPT
    python generate_outputs.py --pdf     # PDF only
    python generate_outputs.py --ppt     # PPT only
    python generate_outputs.py --out my_folder/

Architecture:
    generate_report_data.py   ← shared data module (always runs first)
            │
    ┌───────┴────────┐
    │                │
generate_pdf.py   generate_ppt.js
(reportlab)       (pptxgenjs via Node)

Any change to data/*.json or sizing_engines/*.py is automatically
reflected in both outputs on the next run.
"""
...
def run_pdf(out_dir: Path):
    """Run the Python PDF generator."""
    from generate_pdf import generate_pdf
    out = out_dir / "HCRecov_Report.pdf"
    generate_pdf(out)
    return out


def run_ppt(out_dir: Path):
    """
    Run the Node.js PPT generator.
    First writes _payload.json so Node can read the data payload.
    """
    # 1. Write payload bridge
    payload = build_report_payload()
    payload_path = ROOT / "_payload.json"
    payload_path.write_text(json.dumps(payload, indent=2))
    print(f"📦  Payload written → {payload_path}")

    # 2. Call node
    js_path = ROOT / "generate_ppt.js"
    ...
    if result.returncode != 0:
        print("❌  PPT generator failed:")
        print(result.stderr)
        return None

    # 3. Clean up bridge file
    payload_path.unlink(missing_ok=True)
    return out_dir / "HCRecov_Presentation.pptx"


def main():
    parser = argparse.ArgumentParser(
        description="Generate HCRecov PDF and/or PPT reports"
    )
    parser.add_argument("--pdf", action="store_true", help="Generate PDF only")
    parser.add_argument("--ppt", action="store_true", help="Generate PPT only")
    parser.add_argument(
        "--out", default="outputs",
    ...
    do_pdf = args.pdf or not (args.pdf or args.ppt)
    do_ppt = args.ppt or not (args.pdf or args.ppt)

    print("\n" + "=" * 60)
    print("  HCRecov Report Generator")
    print("=" * 60)
    ...
    if do_pdf:
        ...
        results["pdf"] = run_pdf(out_dir)

    if do_ppt:
        print("\n[2/2]  Generating PowerPoint …")
        results["ppt"] = run_ppt(out_dir)
...
def ledger_table_for_tech(tech_key: str, ledger: dict, styles: dict) -> list:
    ...
    header = ["Component", "Rating", "Max Capacity", "Used Capacity", "CAPEX (USD)", "OPEX ($/hr)"]
    table_data = [header]
    for row in rows:
        table_data.append([
    ...
    col_w = [3.6*cm, 4.2*cm, 3.0*cm, 3.4*cm, 2.3*cm, 2.0*cm]
...
// ===========================================================================
{
  ...
  techKeys.forEach((key, idx) => {
    ...
    const headerRow = ["Component", "Rating", "Max Capacity", "Used Capacity", "CAPEX", "OPEX/hr"].map(h => ({
    ...
    slide.addTable([headerRow, ...dataRows, totalRow], {
      x: 0.4, y: 1.15, w: 9.2,
      colW: [1.9, 2.5, 1.65, 1.65, 0.85, 0.85],
...
```py
"""
generate_report_data.py  —  HCRecov Report Data Module
=======================================================
Single source of truth for all report content.
Both generate_pdf.py and generate_ppt.py import from here.

This module calls the REAL sizing engines (sizing_engines/*.py) directly —
it does not hardcode any engineering numbers. Any change to JSON data files
or sizing engine logic is automatically picked up on the next run, with
zero edits needed in this file or in the report scripts.
"""
...
ENGINES_DIR = PROJECT_ROOT / "sizing_engines"

# Make sure sizing_engines is importable
...
# ---------------------------------------------------------------------------
# Dynamic JSON loader — re-reads on every call so changes are always live
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Component ledgers — runs the REAL sizing engines, no hardcoded numbers
# ---------------------------------------------------------------------------
def get_component_ledgers() -> dict:
    """
    Runs all three sizing engines against the live feed conditions and
    returns their component-level breakdowns exactly as engineering would
    see them: component name, mechanical rating, max vs used capacity,
    flow, CAPEX, OPEX, and the efficiency/kinetics note for each item.

    This is a direct passthrough of simulate_*_rated() — if the engines
    change (new component, corrected formula, new JSON parameter), this
    function picks it up automatically with no edits required here.
    """
    feed = get_feed_conditions()

    membrane_rows   = simulate_membrane_rated(
        feed["feed_mass_kg_hr"], feed["p_feed_bar"], feed["t_feed_k"],
        feed["y_n2"], feed["y_hc"]
    )
    adsorption_rows = simulate_adsorption_rated(
        feed["feed_mass_kg_hr"], feed["p_feed_bar"], feed["y_n2"], feed["y_hc"],
        t_feed_k=feed["t_feed_k"]
    )
    absorption_rows = simulate_absorption_rated(
        feed["feed_mass_kg_hr"], feed["p_feed_bar"], feed["t_feed_k"],
        feed["y_n2"], feed["y_hc"]
    )
    ...
    def _totals(rows):
        return {
            "capex_total": sum(r["capex"] for r in rows),
            "opex_total":  sum(r["opex"] for r in rows),
        }

    def _extract_recovery_pct(rows, keyword):
        """Pull the net recovery % out of the efficiency string of the
        row that reports it (membrane/adsorption/absorption each name
        their own headline recovery metric differently)."""
        import re
        for row in rows:
            m = re.search(r"(?:" + keyword + r")[^:]*:\s*([\d.]+)%", row["efficiency"])
            if m:
                return float(m.group(1))
        return None

    membrane_recovery   = _extract_recovery_pct(membrane_rows,   "Recovery")
    adsorption_recovery = _extract_recovery_pct(adsorption_rows, "Affinity")
    absorption_recovery = _extract_recovery_pct(absorption_rows, "Yield")

    return {
        "membrane": {
            "title": "Hollow-Fiber Polymeric Membrane Skid System",
            "rows": membrane_rows,
            "recovery_pct": membrane_recovery,
            **_totals(membrane_rows),
        },
        "adsorption": {
            "title": "Twin-Bed Cyclic Pressure Swing Adsorption Skid",
            "rows": adsorption_rows,
            "recovery_pct": adsorption_recovery,
            **_totals(adsorption_rows),
        },
        "absorption": {
            "title": "Heavy Hydrocarbon Gas Absorption Recirculation Loop",
            "rows": absorption_rows,
            "recovery_pct": absorption_recovery,
            **_totals(absorption_rows),
        },
    }


# ---------------------------------------------------------------------------
# Feed conditions — sourced from process_parameters.json
# ---------------------------------------------------------------------------
def get_feed_conditions() -> dict:
    """
    Returns the boundary conditions used in all three engine assessments.
    Edit process_parameters.json to change feed specs; this auto-updates.
    """
    # These are the plant inputs — keep them here as the single definition
    return {
        "feed_mass_kg_hr": 650.0,
        "p_feed_bar":      1.20,
        "t_feed_k":        337.15,   # 64 °C
        "y_n2":            0.95,
        "y_hc":            0.05,
        "description":     "PP Purge Vent Gas  |  650 kg/hr  |  1.20 bar(a)  |  64 °C  |  95/5 mol% N2/C3H6",
    }
...
# ---------------------------------------------------------------------------
# Chart data — derived from generate_charts.py values (single source)
# ---------------------------------------------------------------------------
def get_chart_data() -> dict:
    """
    Returns structured chart data for embedding in PDF / PPT, derived
    live from the sizing engine outputs (get_component_ledgers).
    """
    ledgers = get_component_ledgers()
    ...
    efficiencies = [
        ledgers["membrane"]["recovery_pct"],
        ledgers["adsorption"]["recovery_pct"],
        ledgers["absorption"]["recovery_pct"],
    ]
    capex_usd = [
        ledgers["membrane"]["capex_total"],
        ledgers["adsorption"]["capex_total"],
        ledgers["absorption"]["capex_total"],
    ]
    opex_usd_hr = [
        ledgers["membrane"]["opex_total"],
        ledgers["adsorption"]["opex_total"],
        ledgers["absorption"]["opex_total"],
    ]

    target_pct = 90.0
    status = ["PASS" if e >= target_pct else "FAIL ❌" for e in efficiencies]

    return {
        "technologies":    technologies,
        "efficiencies":    efficiencies,
        "capex_usd":       capex_usd,
        "opex_usd_hr":     opex_usd_hr,
        "target_pct":      target_pct,
        "status":          status,
        "chart_min_target_label": f"Min. Target: {target_pct:.0f}%",
    }


# ---------------------------------------------------------------------------
# Comparison matrix — sourced from JSON where possible
# ---------------------------------------------------------------------------
def get_comparison_matrix() -> list[dict]:
    """
    Returns a list of dicts representing the technology comparison table.
    Add new criteria here; both PDF and PPT pick them up automatically.

    Note: "Meets 90% Target" and "Recommended" are derived live from
    chart_data (engine output). Operability / Maintenance / Footprint
    remain engineering-judgement qualitative scores — update them here
    if the design basis changes.
    """
    chart = get_chart_data()
    techs = chart["technologies"]
    effs  = chart["efficiencies"]
    capex = chart["capex_usd"]
    target = chart["target_pct"]

    meets_target = [e >= target for e in effs]

    criteria = [
        {
            "criterion": "Recovery Efficiency (%)",
            "values":    [f"{e:.1f}%" for e in effs],
            "best_idx":  effs.index(max(effs)),
        },
        {
            "criterion": "CAPEX (USD k)",
            "values":    [f"${c/1000:,.1f}k" for c in capex],
            "best_idx":  capex.index(min(capex)),
        },
        {
            "criterion": "Meets 90% Target",
            "values":    ["Yes" if m else "No ❌" for m in meets_target],
            "best_idx":  None,
        },
        {
            "criterion": "Operability",
        ...
        {
            "criterion": "Maintenance",
            "values":    ["Low", "Medium", "Medium"],
            "best_idx":  None,
        },
        {
            "criterion": "Footprint",
            "values":    ["Medium", "Large", "Large"],
            "best_idx":  None,
        },
        {
            # Recommended = meets target AND is the lowest-CAPEX option that does so
            "criterion": "Recommended",
            "values":    [
                "✓ YES" if meets_target[i] and capex[i] == min(c for c, m in zip(capex, meets_target) if m)
                else ("✓ YES" if meets_target[i] else "✗ NO")
                for i in range(len(techs))
            ],
            "best_idx":  None,
        },
    ]
    return {"headers": techs, "rows": criteria}


# ---------------------------------------------------------------------------
# Summary bullets — engineering recommendation text
# ---------------------------------------------------------------------------
def get_summary() -> dict:
    """
    Returns the executive summary and recommendations.
    Findings and the recommendation are generated live from chart_data —
    if engine numbers change (e.g. PSA starts passing the 90% target),
    the narrative updates automatically rather than reading stale text.
    """
    chart = get_chart_data()
    conds = get_feed_conditions()

    names = ["Polymeric Membrane", "Pressure Swing Adsorption", "Heavy Gas Absorption"]
    effs  = chart["efficiencies"]
    capex = chart["capex_usd"]
    target = chart["target_pct"]
    meets_target = [e >= target for e in effs]
    ...
    findings = []
    for name, eff, cap, ok in zip(names, effs, capex, meets_target):
        verdict = "meets target" if ok else f"below the {target:.0f}% plant minimum threshold"
        findings.append(
            f"{name} achieves {eff:.1f}% recovery — {verdict}; "
            f"CAPEX ${cap/1000:,.1f}k."
        )

    # Recommendation: cheapest option among those that meet the target;
    # if none meet target, flag that no option is currently viable.
    passing = [(n, e, c) for n, e, c in zip(names, effs, capex) if e >= target]
    if passing:
        best = min(passing, key=lambda x: x[2])
        others_passing = [p for p in passing if p[0] != best[0]]
        rec = f"{best[0]} is the primary recommendation, meeting the recovery target at the lowest CAPEX (${best[2]/1000:,.1f}k)."
        if others_passing:
            alt_names = ", ".join(o[0] for o in others_passing)
            rec += f" {alt_names} also meet{'s' if len(others_passing)==1 else ''} the target and remain{'s' if len(others_passing)==1 else ''} viable alternatives."
        failing = [n for n, e, c in zip(names, effs, capex) if e < target]
        if failing:
            rec += f" {', '.join(failing)} {'does' if len(failing)==1 else 'do'} not meet the {target:.0f}% target under these feed conditions."
    else:
        rec = (f"No technology meets the {target:.0f}% recovery target under current feed "
               f"conditions — design conditions or target should be revisited.")
    ...
    return {
        ...
        "scope":   (
            f"First-principles assessment of three hydrocarbon recovery "
            f"technologies for a {conds['feed_mass_kg_hr']:.0f} kg/hr purge stream "
            f"at {conds['p_feed_bar']:.2f} bar(a) and "
            f"{conds['t_feed_k'] - 273.15:.0f} °C "
            f"({conds['y_hc']*100:.0f} mol% C3H6)."
        ),
        "findings":       findings,
        "recommendation": rec,
        "prepared_by": "Summer Internship — Batch 2026",
...
# ---------------------------------------------------------------------------
# Master report payload — single call returns everything both scripts need
# ---------------------------------------------------------------------------
def build_report_payload() -> dict:
    """
    Returns a single dict with all data needed to build PDF and PPT reports.
    This is the only function both generator scripts need to call.
    """
    return {
        "summary":    get_summary(),
        "feed":       get_feed_conditions(),
...
if __name__ == "__main__":
    import pprint
    payload = build_report_payload()
    pprint.pprint(payload, depth=3)
    print(f"\n✅ Report payload built successfully — {payload['generated']}")
